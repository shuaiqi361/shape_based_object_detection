...
def detect_folder(folder_path, model_path, data_set, meta_data_path, output_path, output_file=None):
    ...
    for frame_id in range(1200):
        ...
        annotated_image, time_pframe, frame_info = detect_image(frame, model, 0.15, 0.3, 200,
...
if __name__ == '__main__':
    ...
    if sys.argv[1] == '--video':
        ...
        output_path = os.path.join(root_path, 'live_results/Hwy7')
        output_file = os.path.join(output_path, 'RefineDetBOFTraffic3_frames_results.txt')
        detect_folder(folder_path, model_path, data_set, meta_data_path, output_path, output_file)
