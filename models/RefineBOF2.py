...
class VGGBase(nn.Module):
    ...
    def forward(self, image):
        ...
        out = self.mish(self.conv6(out))  # (N, 1024, 32, 32)

        conv7_feats = self.mish(self.conv7(out))  # (N, 1024, 32, 32)
...
class AuxiliaryConvolutions(nn.Module):
    ...
    def forward(self, conv7_feats):
        ...
        """
        out = self.mish(self.conv8_1(conv7_feats))  # (N, 256, 16, 16)
        out = self.mish(self.conv8_2(out))
        conv8_2_feats = out

        out = self.mish(self.conv9_1(out))  # (N, 128, 8, 8)
...
class RefineDetBofLoss2(nn.Module):
    """
    ...
    def __init__(self, priors_cxcy, config, threshold=0.5, neg_pos_ratio=3, theta=0.01):
        super(RefineDetBofLoss2, self).__init__()
