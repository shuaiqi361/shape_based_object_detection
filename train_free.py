...
def main():
    ...
    if args.recover:
        ...
        if args.finetune:
            ...
            for param_tensor in init_model.state_dict().keys():
                if param_tensor.startswith('aux_convs.') or param_tensor.startswith('fpn.'):
    ...
    model = model.to(config.device)
    criterion = criterion(locations=model.locations, config=config).to(config.device)
...
def train(train_loader, model, criterion, optimizer, epoch, config):
    ...
    for i, (images, boxes, labels, _, _) in enumerate(train_loader):
        ...
        # Forward prop.
        predicted_locs, predicted_scores, predicted_centerness = model(images)

        # Loss
        loss = criterion(predicted_locs, predicted_scores, predicted_centerness, boxes, labels) / config.num_iter_flag
...
def evaluate(test_loader, model, optimizer, config):
    ...
    with torch.no_grad():
        ...
        for i, (images, boxes, labels, _, difficulties) in enumerate(tqdm(test_loader, desc='Evaluating')):
            ...
            predicted_locs, predicted_scores, predicted_centerness = model(images)
            decoded_locs, decoded_scores = model.postprocess(predicted_locs, predicted_scores, predicted_centerness)
            ...
            if config.data_name.upper() == 'COCO':
                det_boxes_batch, det_labels_batch, det_scores_batch = \
                    detect(decoded_locs,
                           ...
                           top_k=config.nms['top_k'], priors_cxcy=None,
                           config=config)
            elif config.data_name.upper() == 'VOC':
                det_boxes_batch, det_labels_batch, det_scores_batch = \
                    detect(decoded_locs,
